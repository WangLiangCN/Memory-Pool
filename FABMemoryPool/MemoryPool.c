 ...
 *
 * @note When using this pool in project, less chunk is better in performance, avoid too many search,
 * so is better to set _uFirstChunkBlocks as number of normal memory block when running, and
 * _uGrowChunkBlocks sets to number of maximum memory block using when running.
